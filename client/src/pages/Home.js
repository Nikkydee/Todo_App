...
import { fetchUsersTodos, editTodos, newTodo, removeTodo } from "../utils/todo.utils"
  ...
  const confirmChange = () => {
    const updatedTodos = todos.map(todo => {
      if (todo.task === selectedTaskToEdit) {
        editTodos(newEditTask, todo._id, authToken)
            ...
            return (
              <li key={`todo-${index}`}>
                ...
                <div className="todo-options-container">
                  <label htmlFor="edit-modal-input" onClick={() => editTodo(todo.task, todo._id)} title="Edit" className="icon-center-button">
